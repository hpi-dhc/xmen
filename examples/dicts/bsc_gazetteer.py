...
def get_concept_details(cfg) -> dict:
    path = cfg.dict.custom.gazetteer_path
    ...
    if meta_path := cfg.dict.custom.get("umls_meta_path"):
        ...
        logger.info(f"Extending Gazetteer by UMLS synonyms from {meta_path} in languages {langs}")
        snomed2cui = read_snomed2cui_mapping(meta_path)
        logger.info(f"Number of SCTIDs in UMLS: {len(snomed2cui)}")
